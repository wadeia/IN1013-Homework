...
WHERE b.bill_total > 450.00
  AND b.table_no IN (
      ...
      WHERE rm.headwaiter = (SELECT staff_no FROM restStaff WHERE first_name = 'Charles')
  );
...
WHERE s.staff_no = (
    ...
    JOIN restRest_table rt ON rm.room_name = rt.room_name
    WHERE rt.table_no = (
        SELECT table_no FROM restBill WHERE cust_name = 'Nerida Smith' AND bill_date = 160111
    )
);
...
-- 5
SELECT b.cust_name, hs.first_name AS headwaiter_name, hs.surname AS headwaiter_surname, rt.room_name
FROM restBill b
